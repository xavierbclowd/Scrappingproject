...
class BclowdSpiderPipeline(object):
    ...
    def open_spider(self, spider):
        ...
        self.client = document_client.DocumentClient(
            self.docdb_host, {"masterKey": self.docdb_mkey}
        )
        # create a database if not yet created
        database_definition = {"id": self.docdb_db}
        databases = list(
            self.client.QueryDatabases(
                {
                    "query": "SELECT * FROM root r WHERE r.id=@id",
                    "parameters": [{"name": "@id", "value": database_definition["id"]}],
                }
            )
        )
        feeddb = None
    ...
    def remove_null_empty_fields(self, data):
        if isinstance(data, dict):
            ...
            return {
                key: self.remove_null_empty_fields(value)
            ...
            return [
                self.remove_null_empty_fields(item)
                for item in data
    ...
    def process_item(self, item, spider):
        ...
        docid = BclowdSpiderPipeline.gen_docid_from_string(
            item["domain"] + item["sku"] + id_time_stamp
        )
        document_definition = {
            "id": docid,
            ...
            "collection_name": (
                [item["collection_name"]] if item.get("collection_name") else []
            ),
            "season": [item["season"]] if item.get("season") else [],
            "brand": [item["brand"]] if item.get("brand") else [],
            "product_badge": (
                [item["product_badge"]] if item.get("product_badge") else []
            ),
            ...
            "main_material": (
                [item["main_material"]] if item.get("main_material") else []
            ),
            "secondary_material": (
                [item["secondary_material"]] if item.get("secondary_material") else []
            ),
            "image_url": item["image_url"] if item.get("image_url") else [],
            "size_dimensions": (
                item["size_dimensions"] if item.get("size_dimensions") else []
            ),
            "content": [item["content"]] if item.get("content") else [],
            "specification": (
                [item["specification"]] if item.get("specification") else []
            ),
        ...
        if len(documents) < 1:
            ...
            created_document = self.client.CreateDocument(
                self.feedcoll["_self"], clean_document_definition
            )
            return item
...
class AzureImageUploadPipeline:
    def __init__(
        self, azure_account_name, azure_account_key, container_name, max_chunk_size
    ):
    ...
    def from_crawler(cls, crawler):
        ...
        max_chunk_size = crawler.settings.get(
            "AZURE_BLOB_MAX_CHUNK_SIZE", 4 * 1024 * 1024
        )
        return cls(
            azure_account_name, azure_account_key, container_name, max_chunk_size
        )
